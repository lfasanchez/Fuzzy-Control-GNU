...
% The amplitud factor of the membership functions is defined:
...
FacRedQT1 = (FacRedQS + FacRedQL)/2;%The reduction factor for T1 fuzzy system is calculated regarding that its  alpha = 1
...
FacRed16T1 = (FacRed16S + FacRed16L)/2;%The reduction factor for T1 fuzzy system is calculated regarding that its  alpha = 1
...
FacRed64T1 = (FacRed64S + FacRed64L)/2;%The reduction factor for T1 fuzzy system is calculated regarding that its  alpha = 1
...
FacRed256T1 = (FacRed256S + FacRed256L)/2;%The reduction factor for T1 fuzzy system is calculated regarding that its  alpha = 1
...
% T1 boundary points:
BQTRMS = [TQ1S,TQ1MS,TQ1MI,TQ1I];
MQTRMS = [TQ2S,TQ2MS,TQ2MI,TQ2I];
...
% T1 boundary points:
B16TRMS = [T161S,T161MS,T161MI,T161I];
R16TRMS = [T162S,T162MS,T162MI,T162I];
M16TRMS = [T163S,T163MS,T163MI,T163I];
...
% T1 boundary points:
B64TRMS = [T641S,T641MS,T641MI,T641I];
R64TRMS = [T642S,T642MS,T642MI,T642I];
M64TRMS = [T643S,T643MS,T643MI,T643I];
...
% T1 boundary points:
B256TRMS = [T2561S,T2561MS,T2561MI,T2561I];
M256TRMS = [T2562S,T2562MS,T2562MI,T2562I];


% The input variables are normalized and fuzzified for the UMF, LMF and the T1 MF:
[ut11, ut12, ut13, ut21, ut22, ut23, ut24] = Fuzzy(FacRedQT1,FacRed16T1,FacRed64T1,FacRed256T1, RMS, ModAct,BQTRMS,MQTRMS,B16TRMS,R16TRMS,M16TRMS,B64TRMS,R64TRMS,M64TRMS,B256TRMS,M256TRMS);

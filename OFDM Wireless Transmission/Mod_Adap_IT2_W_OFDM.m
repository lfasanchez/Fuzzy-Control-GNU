...
ModAct =  0;% Actual Modulation (Input to the system)  0 - QPSK; 1 - 16QAM; 2 - 64QAM; 3- 256QAM 
Dig_Gain = 0.7;% Digital Gain given by the transmitter
SNR = 5; %SNR Measured
...
% The input variables are normalized and fuzzified for UMF and the LMF:
...
[ul11,ul12,ul13,ul21,ul22,ul23,ul24,ul31,ul32,ul33] = Fuzzy(FacRedQL,...
 ...
 %The antecedents of the rusles are calcuslated for UMF:
...
%The antecedents of the rulles Are calcullated for LMF:
...
% The mandami implication is calculated for the UMF:
...
% The mandami implication is calculated for the LMF:
...
%The maximum value of the implications are calculated for the UMF:
...
%The maximum value of the implications are calculated for the LMF:
...
xlabel(ax1,'x')
ylabel(ax1,'f(x)')
legend('New Modulation - Output UMF','New Modulation - Output LMF' )
...
xlabel(ax2,'x')
ylabel(ax2,'f(x)')
legend('New Gain - Output UMF','New Gain - Output LMF' )
%}


% The  EIASC method is used for defuzzification in modulation :
outnm = EIASC(outs1,outl1,x,1);
% The  EIASC method is used for defuzzification in digital gain :
outnDG = EIASC(outs2,outl2,x,1);
toc
pler = toc;
...
function [S1S,S1MS,S1MI,S1I,S2S,S2MS,S2MI,S2I,S3S,S3MS,S3MI,S3I,L1S,L1MS,...
    L1MI,L1I,L2S,L2MS,L2MI,L2I,L3S,L3MS,L3MI,L3I] = Limits3(LimI, LimS,DistCent)
